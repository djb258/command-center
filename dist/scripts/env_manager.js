...
class EnvironmentManager {
  constructor() {
    ...
    this.envVariables = [
      ...
      },
      {
        ...
        description: 'Abacus.AI API key for ML platform',
        required: false,
        category: 'AI APIs',
      },
      {
      ...
      },
      {
        ...
        description: 'Genspark AI search engine API key',
        required: false,
        category: 'AI APIs',
      },
      {
        key: 'RTRVR_API_KEY',
        description: 'RTRVR.AI retrieval and search API key',
        required: false,
        category: 'AI APIs',
      },
      {
        key: 'BROWSERLESS_API_KEY',
        description: 'Browserless.ai headless browser API key',
        required: false,
        category: 'AI APIs',
      },
      {
      ...
      {
        key: 'SYNC_SSH_KEY_PATH',
        description: 'SSH key path for machine sync',
        required: false,
        category: 'Machine Sync',
        example: '~/.ssh/id_rsa',
      },
      {
        key: 'CURSOR_SETTINGS_PATH',
        description: 'Cursor settings file path',
        required: false,
        category: 'Machine Sync',
      },
    ];
  ...
  async validateEnvironment() {
    ...
    for (const variable of this.envVariables) {
      ...
      if (variable.required && (!value || value.startsWith('your-'))) {
        this.log(
          `❌ Missing required: ${variable.key} - ${variable.description}`,
          'red'
        );
        issues++;
      } else if (
        value &&
        variable.validation &&
        !variable.validation.test(value)
      ) {
        this.log(
          `⚠️ Invalid format: ${variable.key} - Expected format not matched`,
          'yellow'
        );
        warnings++;
    ...
    this.log(
      `✅ Configured variables: ${Object.keys(envVars).length}`,
      'green'
    );
  ...
  async listEnvironmentVariables() {
    ...
    for (const category of categories) {
      this.log(`\n🔸 ${category}:`, 'yellow');
      const categoryVars = this.envVariables.filter(
      ...
      for (const variable of categoryVars) {
        ...
        const example = variable.example
          ...
          : '';
        this.log(
          `  ${variable.key} ${required} - ${variable.description}${example}`
        );
      }
    }
  }
...
`;
  ...
  checkCommonIssues(envVars) {
    ...
    if (placeholderCount > 0) {
      this.log(
        `⚠️ Found ${placeholderCount} placeholder values that need to be replaced`,
        'yellow'
      );
    }
...
//# sourceMappingURL=env_manager.js.map
